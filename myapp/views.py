...
def dataloader(request):
    global Face_data, Person_data, flann, trainy, pca
        ...
        if len(faces) > 0:
            ...
            for (x, y, w, h) in faces:
                ...
                if x > 0 and y > 0:
                    ...
                    print("Embedding shape is new1: ", embedding.shape)
                    # embedding = np.reshape(embedding, (1, 1280))
                    embedding = np.expand_dims(embedding, axis=0)

                    print("Embedding shape is new2: ", embedding.shape)
                    embedding = pca.transform(embedding)
                    print("Embedding shape is new3: ", embedding.shape)
...
def prepare_images(request):
    global Face_data, Images_preprocessed, person_face, Person_data, flann, trainy, pca
    ...
    if request.method == 'POST':
        ...
        if name:
            ...
            for captures in Images_preprocessed:
                ...
                embeddings = model.predict(captures)
                embedding = embeddings.flatten()
            ...
            print("Embedding shape is prepare : ", trainX.shape)
